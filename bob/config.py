...
class ClaudeConfig(BaseSettings):
    ...
    model: str = Field("claude-sonnet-4-5-20250929", alias="BOB_MODEL")
    max_tokens: int = Field(8192, alias="BOB_MAX_TOKENS")
    thinking_budget: int = Field(16000, alias="BOB_THINKING_BUDGET")
...
class MemoryConfig(BaseSettings):
    ...
    data_dir: Path = Field(Path("./bob_data"), alias="BOB_DATA_DIR")
    episodic_db_path: Path = Field(Path("./bob_data/episodic.db"), alias="BOB_EPISODIC_DB")
    semantic_db_path: Path = Field(Path("./bob_data/semantic_vectors"), alias="BOB_SEMANTIC_DB")

    # Working memory constraints (Miller's 7±2)
    working_memory_slots: int = Field(7, alias="BOB_WORKING_MEMORY_SLOTS")
...
class HeartbeatConfig(BaseSettings):
    ...
    interval: float = Field(30.0, alias="BOB_HEARTBEAT_INTERVAL")
    min_interval: float = Field(5.0, alias="BOB_HEARTBEAT_MIN_INTERVAL")
    max_interval: float = Field(120.0, alias="BOB_HEARTBEAT_MAX_INTERVAL")
...
class AffectConfig(BaseSettings):
    ...
    arousal_ceiling: float = Field(0.92, alias="BOB_AROUSAL_CEILING")
    distress_threshold: float = Field(0.80, alias="BOB_DISTRESS_THRESHOLD")
    distress_duration_limit: float = Field(300.0, alias="BOB_DISTRESS_DURATION_LIMIT")
...
class ContextConfig(BaseSettings):
    ...
    context_limit: int = Field(180000, alias="BOB_CONTEXT_LIMIT")
    compaction_trigger: float = Field(0.85, alias="BOB_COMPACTION_TRIGGER")
...
class SafetyConfig(BaseSettings):
    ...
    max_tool_iterations: int = Field(25, alias="BOB_MAX_TOOL_ITERATIONS")
    require_approval_for: list[str] = Field(
        default_factory=lambda: ["file_write", "shell_exec", "web_request"],
        alias="BOB_REQUIRE_APPROVAL_FOR",
    )
    sandbox_enabled: bool = Field(True, alias="BOB_SANDBOX_ENABLED")
...
class MCPConfig(BaseSettings):
    ...
    servers: str = Field("[]", alias="BOB_MCP_SERVERS")
...
class BobConfig:
    ...
    def __repr__(self) -> str:
        return (
            f"BobConfig(model={self.claude.model}, "
