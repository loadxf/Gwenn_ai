"""REPL — GwennSession and the interactive terminal interface.

The interactive REPL is the default mode when ``gwenn`` is invoked without
a subcommand.  This module owns:

* :class:`GwennSession` — the full REPL lifecycle (init → interact → shutdown)
* REPL constants (slash-commands, output styles)
* Channel-error helpers used during startup
* ``_connect_daemon_channel()`` — async daemon connector
* ``run_repl()`` — convenience entry-point called by the Click CLI
"""
...
def run_repl(ctx_obj: dict[str, Any] | None = None) -> None:
    ...
    Called by the Click CLI when no subcommand is given.  Handles
    ``--channel`` and ``--no-daemon`` flags from the CLI context.
    """
    ...
    use_daemon = True
    if ctx_obj:
        channel_override = ctx_obj.get("channel")
        use_daemon = not ctx_obj.get("no_daemon", False)
